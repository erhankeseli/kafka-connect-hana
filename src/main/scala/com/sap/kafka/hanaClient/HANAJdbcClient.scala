...
case class HANAJdbcClient(hanaConfiguration: HANAConfig)  {
  ...
  def createTable(namespace: Option[String],
                  ...
                  partitionType: String = Parameters.NO_PARTITION,
    ...
    var partition = ""
    if (partitionType == Parameters.HASH_PARTITION) {
      if (keys == null) {
        throw new HANAClientException("Hash Partition is not supported when keys are" +
          " not specified. Provide the 'pk.mode' parameter")
      } else {
        partition = "PARTITION BY HASH(\"" + keys.mkString("\", \"") + "\") PARTITIONS " + partitionCount
      }
    } else if (partitionType == Parameters.ROUND_ROBIN_PARTITION) {
      partition = "PARTITION BY ROUNDROBIN PARTITIONS " + partitionCount
    }

    val query = if (columnTable) s"""CREATE COLUMN TABLE $fullTableName ($fixedSchema$primaryKeys) $partition"""
    else s"""CREATE TABLE $fullTableName ($fixedSchema$primaryKeys) $partition"""
